...
- Handshake `discovery.bridgedTools` returns tool names only. `discovery.toolDefinitionsTool` describes the local `lmToolsBridge_getToolDefinitions` helper plus input/output schemas. Before invoking a bridged tool, cache that tool's ToolDefinition once with the helper when no valid cached definition exists. Request multiple likely-needed tool definitions in one helper call when possible, then reuse valid cached definitions unless missing or suspected stale. It is available only after `lmToolsBridge_bindWorkspace` succeeds and is not callable through `lmToolsBridge_callBridgedTool`.
...
- 握手里的 `discovery.bridgedTools` 只返回 tool name. `discovery.toolDefinitionsTool` 会包含本地 helper `lmToolsBridge_getToolDefinitions` 的用法说明和 input/output schema. 调用某个 bridged tool 前, 如果没有有效缓存, 先用 helper 为该 tool 缓存一次 ToolDefinition. 尽量在一次 helper 调用中预取未来可能用到的多个 ToolDefinition 并缓存, 缓存仍有效时不要再次请求同一个 tool, 除非缓存缺失或疑似过期. 它只能在 `lmToolsBridge_bindWorkspace` 成功后调用,且不能通过 `lmToolsBridge_callBridgedTool` 包装调用.
