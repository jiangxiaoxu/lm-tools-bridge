...
- The direct `lmToolsBridge_callBridgedTool` helper is documented in `lm-tools://guide`; it requires a short `title` wrapper parameter for the UI and does not pass that title to the target bridged tool. The names-only discovery resource is `lm-tools://tool-names`, and it returns an actionable bind/rebind error until the workspace is actively bound.
...
- `lmToolsBridge_callBridgedTool` 的详细调用和 fallback 规则已经并入 `lm-tools://guide`; 它要求传入简短的 `title` wrapper 参数用于 UI,且不会把该 title 传给目标 bridged tool. names-only discovery resource 是 `lm-tools://tool-names`,它在未绑定或绑定失效时会返回可执行的 bind/rebind 提示.
