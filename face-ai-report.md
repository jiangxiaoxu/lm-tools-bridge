...
- An additional anonymized brace-scope fixture (`WorkspaceA`, `WorkspaceB`) uses a large real-source-derived corpus under anonymized workspace names; it covers `{WorkspaceA,WorkspaceB}/**/*.{h,cpp,cs,as}` file aggregation, brace-selector normalization, `lm_qgrepSearchText.includePattern` filtering, literal text-search pipe/quoted-pipe/whitespace/drop-empty-whitespace-branch/fallback handling, and regex text-search integration (smart-case and explicit case-sensitive) across the selected workspaces only.
