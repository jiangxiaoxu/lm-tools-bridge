...
test('pathScope spec text documents shared examples and restrictions', () => {
  ...
  assert.match(text, /GameWorkspace\/Script\/\*\*\/\*\.as/u);
  assert.match(text, /\{GameWorkspace,UE5\}\/\*\*\/\*\.\{h,cpp,as\}/u);
  assert.match(text, /\{GameWorkspace\/Script\/\*\*\/\*\.as,UE5\/Engine\/\*\*\/Source\/\*\*\/\*\.h\}/u);
  ...
  assert.match(text, /GameWorkspace\|UE5\/\*\*\/\*\.as/u);
});
