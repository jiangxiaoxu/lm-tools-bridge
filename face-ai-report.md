...
- In glob mode for `lm_qgrepSearchText.query`, simple top-level bare `|` mistakes such as `A|B|C` are auto-normalized to brace globs such as `{A,B,C}` with a text-output note plus `effectiveQuery`; non-top-level or otherwise complex bare `|` usage fails fast with guidance to use brace globs or `querySyntax='regex'`.
...
- qgrep-failure: verify `lm_qgrepSearchText` outside-workspace `includePattern` returns expected error, simple top-level bare `|` text queries auto-normalize with note plus `effectiveQuery`, complex non-top-level bare `|` text queries fail fast with guidance, `lm_findFiles.query` rejects bare `|` alternation, legacy `searchPath`/`includeIgnoredFiles` on text search are ignored, `lm_qgrepSearchFiles` rejects bare `|` alternation in glob mode plus legacy `mode`/`searchPath`/`isRegexp` while ignoring `includeIgnoredFiles`, no-init qgrep search/files auto-initialize then wait for readiness instead of failing immediately, and file-search summaries keep `count/totalAvailable/capped/hardLimitHit/maxResultsApplied` self-consistent.
