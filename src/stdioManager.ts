...
function getRequestWorkspaceTitleParamMessage(): string {
  return appendNextStep(
    'Invalid params: expected params.title (non-empty string).',
    `call ${REQUEST_WORKSPACE_METHOD} with a short user-facing title describing the workspace bind.`,
  );
}
...
function createServer(): { server: Server; cleanup: () => void } {
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    if (name === REQUEST_WORKSPACE_METHOD) {
      ...
      if (!title) {
        throw new McpError(ErrorCode.InvalidParams, getRequestWorkspaceTitleParamMessage());
      }
