...
5. After bind succeeds, fetch needed ToolDefinitions with `lmToolsBridge_getToolDefinitions`, then use bridged workspace tools from `tools/list`, or call `lmToolsBridge_callBridgedTool`.
...
- Handshake `discovery.bridgedTools` returns tool names only. `discovery.toolDefinitionsTool` describes the local `lmToolsBridge_getToolDefinitions` helper plus input/output schemas; before calling a bridged tool, call that helper if the ToolDefinition has not already been fetched, batching likely-needed future tool names when possible. It is available only after `lmToolsBridge_bindWorkspace` succeeds and is not callable through `lmToolsBridge_callBridgedTool`.
...
4. `cwd` 需要传绝对路径,可以是:
   ...
   - `.code-workspace` 路径
   - 相对路径无效
5. 绑定成功后,先用 `lmToolsBridge_getToolDefinitions` 获取需要的 ToolDefinition,再通过 `tools/list` 使用桥接后的 workspace tools,或继续调用 `lmToolsBridge_callBridgedTool`.
