...
- `lm_qgrepSearchText.query` defaults to literal semantics; top-level unescaped `|` splits the query into multiple literal branches, only whole branches wrapped by outer double quotes keep `|` literal, malformed/partial double quotes are treated as ordinary characters, unquoted `\|` keeps `|` literal, and empty split branches fall back to matching the entire raw query as one literal string.
...
- `lm_qgrepSearchText` payload includes `querySemanticsApplied` (`literal`, `literal-fallback`, or `regex`) plus `casePolicy`/`caseModeApplied`; `lm_qgrepSearchFiles` payload includes `querySemanticsApplied` and optional `scope`.
...
- `lm_qgrepSearchText` and `lm_qgrepSearchFiles` descriptions do not include the explicit "updating below 100% blocks until ready or 150s timeout" sentence and keep hard-limit wording concise; `lm_qgrepSearchText` descriptions explicitly note default literal matching plus literal-branch `|` splitting, outer-double-quote handling, and empty-branch fallback, and `lm_qgrepSearchFiles` descriptions explicitly note whole-path-anchored path matching instead of substring matching; use `lm_qgrepGetStatus` for readiness details.
...
- An additional anonymized brace-scope fixture (`WorkspaceA`, `WorkspaceB`) uses a large real-source-derived corpus under anonymized workspace names; it covers `{WorkspaceA,WorkspaceB}/**/*.{h,cpp,cs,as}` file aggregation, brace-selector normalization, `lm_qgrepSearchText.includePattern` filtering, literal text-search pipe/quoted-pipe/fallback handling, and regex text-search integration (smart-case and explicit case-sensitive) across the selected workspaces only.
