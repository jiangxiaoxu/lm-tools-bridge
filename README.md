...
- When no workspace qgrep index is initialized yet, `lm_qgrepGetStatus` also includes an auto-initialization hint stating that `lm_qgrepSearchText` and `lm_qgrepSearchFiles` will auto-initialize indexes on actual query calls.
- `lm_qgrepGetStatus` input: no arguments.
- `lm_qgrepSearchText` performs regex-only text search using the extension bundled binary at `bin/qgrep.exe`.
- `lm_qgrepSearchText` uses default smart-case matching: all-lowercase queries run case-insensitive, while queries containing uppercase letters run case-sensitive.
...
- On extension startup, workspaces that already have qgrep initialized (`.vscode/qgrep/workspace.cfg`) automatically queue one background refresh that syncs extension-managed `workspace.cfg` blocks before running `qgrep update` for current-session progress/file totals.
...
- On qgrep init/rebuild, startup refresh of initialized workspaces, and `search.exclude` configuration changes, the extension syncs a managed Unreal include block for `*.ush`/`*.usf`/`*.ini`, a managed PowerShell include block for `*.ps1`, and `search.exclude=true` patterns into a managed `exclude ...` block inside each initialized `workspace.cfg` with fixed excludes for `.git`, `Intermediate`, `DerivedDataCache`, `Saved`, `.vs`, and `.vscode`. `.gitignore` is not synced yet.
...
- Default policy: `lm_qgrepGetStatus`, `lm_qgrepSearchText`, and `lm_qgrepSearchFiles` are exposed and enabled by default (`lm_qgrepSearchText`/`lm_qgrepSearchFiles` are also always-exposed built-ins).
...
- 当还没有任何 workspace 建立 qgrep 索引时,`lm_qgrepGetStatus` 还会返回自动初始化提示,说明调用 `lm_qgrepSearchText` 和 `lm_qgrepSearchFiles` 这类实际查询工具时会自动创建索引.
- `lm_qgrepGetStatus` 输入: 无参数.
- `lm_qgrepSearchText` 使用扩展内置二进制 `bin/qgrep.exe` 执行 regex 文本搜索.
- `lm_qgrepSearchText` 默认使用 smart-case: query 全小写时按大小写不敏感搜索, query 中包含大写字母时按大小写敏感搜索.
...
- 扩展启动时,对于已经初始化过 qgrep(存在 `.vscode/qgrep/workspace.cfg`)的 workspace,会自动排队一次后台刷新: 先同步本插件受管的 `workspace.cfg` 区块,再执行 `qgrep update`,用于补足当前会话的进度/文件总数显示.
...
- 在 qgrep init/rebuild,已初始化 workspace 的启动刷新,以及 `search.exclude` 配置变更时,扩展会把 `*.ush`/`*.usf`/`*.ini` 的受管 Unreal include 区块和 `*.ps1` 的受管 PowerShell include 区块写入每个已初始化 `workspace.cfg`,并把 `search.exclude=true` 规则同步到受管 `exclude ...` 区块,同时固定写入 `.git`、`Intermediate`、`DerivedDataCache`、`Saved`、`.vs`、`.vscode` 排除规则. 当前还不会同步 `.gitignore`.
...
- 默认策略: `lm_qgrepGetStatus`、`lm_qgrepSearchText`、`lm_qgrepSearchFiles` 默认 exposed 且默认 enabled (`lm_qgrepSearchText`/`lm_qgrepSearchFiles` 还属于始终暴露的内置工具).
