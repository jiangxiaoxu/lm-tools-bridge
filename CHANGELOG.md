...
#### Changed
- Disabled `lm_formatFiles` by default while keeping it exposed by default, so enabling formatting remains opt-in.

### 中文

#### 变更
- 将 `lm_formatFiles` 调整为默认禁用并继续保持默认暴露,使格式化能力保持显式启用模式.
...
#### Changed
- Enabled `lm_formatFiles` by default while keeping it exposed by default, so the tool is immediately available without extra enablement steps.

### 中文

#### 变更
- 将 `lm_formatFiles` 调整为默认启用并继续保持默认暴露,使该工具开箱即可使用,无需额外启用步骤.
...
#### Changed
- Bumped the extension package version to `1.0.152` for the next release build.

### 中文

#### 变更
- 将扩展包版本提升到 `1.0.152`,用于下一次发布构建.
...
#### Changed
- Bumped the extension package version to `1.0.148` for the next release.

### 中文

#### Changed
- 将扩展包版本提升到 `1.0.148`,用于下一次发布.
