...
3. Call `lmToolsBridge_bindWorkspace` with a short `title`.
4. Set `cwd` to an absolute path for either:
   - the project path
   - the `.code-workspace` path
   - relative paths are invalid
5. After bind succeeds, use bridged workspace tools from `tools/list`, or call `lmToolsBridge_callBridgedTool`.

Notes:
- Handshake is required before using bridged workspace tools.
- `lmToolsBridge_bindWorkspace` is the entrypoint when the task calls for vscode-tools-like workspace search, code navigation, diagnostics, or VS Code IDE actions, or explicitly includes phrases like `vscode-tools` or `use vscode`.
...
- Handshake `discovery.bridgedTools` returns tool names only. `discovery.toolDefinitionsTool` describes the local `lmToolsBridge_getToolDefinitions` helper plus input/output schemas, including its required wrapper `title`. Before invoking a bridged tool, first use `lmToolsBridge_getToolDefinitions` with a short `title` and only tool names whose ToolDefinitions are unknown; helper-returned definitions include MCP display/UI metadata such as `title`, `_meta`, `annotations`, and `outputSchema`, while omitting VS Code metadata such as `tags`. Do not guess or infer input schemas from tool names, then reuse known ToolDefinitions. It is available only after `lmToolsBridge_bindWorkspace` succeeds and is not callable through `lmToolsBridge_callBridgedTool`.
- Bridged tool descriptors include MCP Apps/OpenAI Apps metadata for the shared `ui://lm-tools-bridge/tool-result-card.html` resource. Tool results still include text `content` fallback plus `structuredContent`; result `_meta` is reserved for component-only hydration data.
- `lmToolsBridge_bindWorkspace`, `lmToolsBridge_getToolDefinitions`, and `lmToolsBridge_callBridgedTool` require a short wrapper `title` parameter for readable MCP UI labels. `lmToolsBridge_callBridgedTool` does not pass that title to the target bridged tool. The names-only discovery resource is `lm-tools://tool-names`, and it returns an actionable bind/rebind error until the workspace is actively bound.
...
3. 调用带简短 `title` 的 `lmToolsBridge_bindWorkspace`.
4. `cwd` 需要传绝对路径,可以是:
   - 项目路径
   - `.code-workspace` 路径
   - 相对路径无效
5. 绑定成功后,通过 `tools/list` 使用桥接后的 workspace tools,或继续调用 `lmToolsBridge_callBridgedTool`.

说明:
- 使用桥接 workspace tools 之前,必须先完成握手.
- 当任务需要类似 vscode-tools 的 workspace search、code navigation、diagnostics 或 VS Code IDE actions,或明确说出 `vscode-tools` 或 `use vscode` 时,应从 `lmToolsBridge_bindWorkspace` 开始.
...
- 握手里的 `discovery.bridgedTools` 只返回 tool name. `discovery.toolDefinitionsTool` 会包含本地 helper `lmToolsBridge_getToolDefinitions` 的用法说明和 input/output schema, 包括必填 wrapper `title`. 调用某个 bridged tool 前, 必须先用带简短 `title` 的 `lmToolsBridge_getToolDefinitions` 获取 ToolDefinition 未知的 exact tool 完整 ToolDefinition; helper 返回的 definition 包含 `title`、`_meta`、`annotations` 和 `outputSchema` 等 MCP display/UI metadata, 但会省略 `tags` 等 VS Code metadata. 不得根据 tool name 猜测或推断 input schema, 后续复用已知 ToolDefinition. 它只能在 `lmToolsBridge_bindWorkspace` 成功后调用, 且不能通过 `lmToolsBridge_callBridgedTool` 包装调用.
- Bridged tool descriptor 会包含指向共享 `ui://lm-tools-bridge/tool-result-card.html` resource 的 MCP Apps/OpenAI Apps metadata. Tool result 仍保留 text `content` fallback 和 `structuredContent`; result `_meta` 只用于 component hydration data.
- `lmToolsBridge_bindWorkspace`、`lmToolsBridge_getToolDefinitions` 和 `lmToolsBridge_callBridgedTool` 都要求传入简短 wrapper `title` 参数用于可读 MCP UI label. `lmToolsBridge_callBridgedTool` 不会把该 title 传给目标 bridged tool. names-only discovery resource 是 `lm-tools://tool-names`, 它在未绑定或绑定失效时会返回可执行的 bind/rebind 提示.
