...
- Generated qgrep regexes (both `workspace.cfg` sync rules and runtime `searchPath` glob filters) are validated to avoid non-capturing groups (`(?:...)`) and other Perl-style `(?...)` constructs because qgrep rejects that syntax.
...
- `lm_qgrepSearchFiles` does not support `searchPath`; passing `searchPath` returns an input error. Files search always runs across all initialized workspaces and keeps qgrep native `files` mode semantics (`fp`/`fn`/`fs`/`ff`).
...
- [qgrep search path rejected] Read: `src/qgrep.ts -> resolveSearchPath | resolveGlobSearchTargets | ensureFilesSearchPathUnsupported`; Decide: `lm_qgrepSearchText.searchPath` still enforces existing path/glob rules, while `lm_qgrepSearchFiles` rejects any `searchPath` input; Verify: text outside/ambiguous path returns expected tool error and files `searchPath` returns unsupported error.
...
- qgrep-failure: verify `lm_qgrepSearchText` outside-workspace `searchPath` returns expected error, `lm_qgrepSearchFiles` rejects `searchPath`, and no-init qgrep search/files auto-initialize then wait for readiness instead of failing immediately.
